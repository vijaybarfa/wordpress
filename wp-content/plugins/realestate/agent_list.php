...
$post_new_file = "admin.php?page=agent_add";
...
function get_columns(){
        $columns = array(
		    ...
		    'agent_id' => __( 'Agent ID', 'mylisttable' ),
            'agent_name' => __( 'Name', 'mylisttable' ),
            'agent_email' => __( 'Email', 'mylisttable' )
        );
         return $columns;
    }
...
function prepare_data() {
  global $wpdb;
  $sql = "SELECT *FROM ".PRO_TABLE_PREFIX."agent where  1";
...
function get_sortable_columns() {
 ...
 function column_cb($item) {
    return sprintf(
    '<input type="checkbox" name="book[]" value="%s" />', $item['agent_id']
    );
 }
  ...
  function column_booktitle($item) {
	...
	$actions = array(
    'edit' => sprintf('<a href="?page=%s&action=%s&book=%s">Edit</a>',$_REQUEST['page'],'edit',$item['agent_id']),
    'delete' => sprintf('<a href="?page=%s&action=%s&book=%s">Delete</a>',$_REQUEST['page'],'delete',$item['agent_id']),
    );
    return sprintf('%1$s %2$s', $item['agent_id'], $this->row_actions($actions) );
  } 
...
//function my_render_list_page(){
  ...
  echo '</pre><div class="wrap">
   ...
   <p class="search-box">
	...
	<h2>Agent Listing'.esc_html( $post_type_object->labels->name ).'<a href="'.$post_new_file.'" class="add-new-h2">Add New</a></h2>';
